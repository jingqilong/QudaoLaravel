...
//        DB::beginTransaction();
//        #如果手机号已经注册，直接进行绑定
//        if ($member = MemberRepository::getOne(['m_phone' => $mobile])){
//            $user_id = $member['m_id'];
...
//                return false;
//            }
//            if (!$referral_user = MemberRepository::getOne(['m_referral_code' => $request['referral_code']])){
...
//                $relation_user = ['parent_id' => 0,'path' => '0,'.$referral_user['m_id'].',','level' => 1];
...
//        return true;
    ...
    public function login($code, $user_arr, $access_token){
        ...
        if ($bind = MemberBindRepository::exists(['identifier' => $openId])){dd($bind);
